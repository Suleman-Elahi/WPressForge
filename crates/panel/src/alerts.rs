...
const RULES: &[Rule] = &[
    Rule { id: "disk.high", severity: "critical" },
    Rule { id: "ssl.expiring", severity: "warning" },
    Rule { id: "site.offline", severity: "critical" },
    Rule { id: "backup.stale", severity: "warning" },
];
...
pub async fn evaluate(state: &crate::state::AppState) {
    ...
    for row in &servers {
        ...
        if row.metrics.disk_percent > 85.0 {
            ...
            insert_if_absent(db, "disk.high", target, &format!(
                "Disk usage on {} is {:.0}%", target, row.metrics.disk_percent
            )).await;
        } else {
    ...
    for row in &sites {
        if let Some(expires) = &row.site.ssl.expires_at {
            ...
            if remaining < Duration::days(14) && remaining > Duration::zero() {
                insert_if_absent(db, "ssl.expiring", &row.site.domain, &format!(
                    "TLS certificate for {} expires in {} days", row.site.domain, remaining.num_days()
                )).await;
            } else {
    ...
    for row in &sites {
        if row.site.status == SiteStatus::Failed {
            insert_if_absent(db, "site.offline", &row.site.domain, &format!(
                "Site {} is in failed state", row.site.domain
            )).await;
        } else {
    ...
    for schedule in &schedules {
        let last_backup = db::sites::last_backup_time(db, schedule.site_id).await.ok().flatten();
        ...
        if stale {
            if let Ok(domain) = db::sites::domain_by_id(db, schedule.site_id).await {
                insert_if_absent(db, "backup.stale", &domain, &format!(
                    "No backup for {domain} in over 48 hours"
                )).await;
            }
